import * as React from 'react';
import PropTypes from 'prop-types';
import Tabs from '@mui/material/Tabs';
import Tab from '@mui/material/Tab';
import Box from '@mui/material/Box';
import StudentsListComponent from './StudentsListComponent'
import PostStudentsForm from './PostStudentsForm'
import StudentsMarksComponent from './StudentsMarksComponent'
import AnnouncementCompo from './AnnouncementCompo'
import InformationPanel from './InformationPanel';

function CustomTabPanel(props) {
    const { children, value, index, ...other } = props;

    return (
        <div
            role="tabpanel"
            hidden={value !== index}
            id={`simple-tabpanel-${index}`}
            aria-labelledby={`simple-tab-${index}`}
            {...other}
        >
            {value === index && <Box sx={{ p: 3 }}>{children}</Box>}
        </div>
    );
}

CustomTabPanel.propTypes = {
    children: PropTypes.node,
    index: PropTypes.number.isRequired,
    value: PropTypes.number.isRequired,
};

function a11yProps(index) {
    return {
        id: `simple-tab-${index}`,
        'aria-controls': `simple-tabpanel-${index}`,
    };
}

export default function BasicTabs() {
    const [value, setValue] = React.useState(0);

    const handleChange = (event, newValue) => {
        setValue(newValue);
    };

    return (
        <Box sx={{ width: '100%' }}>
            <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">
                    <Tab label="Student count" {...a11yProps(0)} />
                    <Tab label="Student list" {...a11yProps(1)} />
                    <Tab label="Add student marks" {...a11yProps(2)} />
                    <Tab label="Student results" {...a11yProps(3)} />
                    <Tab label="Create Announcement" {...a11yProps(4)} />
                </Tabs>
            </Box>
            <CustomTabPanel value={value} index={0}>
                <InformationPanel/>
            </CustomTabPanel>
            <CustomTabPanel value={value} index={1}>
                <StudentsListComponent />
            </CustomTabPanel>
            <CustomTabPanel value={value} index={2}>
                <PostStudentsForm/>
            </CustomTabPanel>
            <CustomTabPanel value={value} index={3}>
                <StudentsMarksComponent/>
            </CustomTabPanel>
            <CustomTabPanel value={value} index={4}>
                <AnnouncementCompo/>
            </CustomTabPanel>
        </Box>
    );
}
